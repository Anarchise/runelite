...
@PluginDescriptor(
		...
		tags = {"net/runelite/client/plugins/zulrah", "zul", "andra", "snakeling", "zhuri/nicole", "girls rule boys drool"},
...
public class ZulrahPlugin extends Plugin implements KeyListener
{
	...
	private void onGameTick(GameTick event) {
		...
		if (data.getCurrentPhasePrayer().isPresent()) {
			...
			if (client.getVar(currentPrayer.getVarbit()) == 0) {
				core.clickPrayer(currentPrayer.getWidgetInfo());
			}
		}
		...
		if (!(event.getActor() instanceof NPC)) 
	{
		...
		if (npc.getName() != null && !npc.getName().equalsIgnoreCase("net/runelite/client/plugins/zulrah"))
	{
			return;
		}
